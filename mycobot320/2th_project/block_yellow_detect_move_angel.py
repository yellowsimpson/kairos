...
# 로봇 팔 각도 이동 후 카메라로 노란색 탐지 및 그리퍼 동작
def perform_action_and_detect_yellow():
    print("로봇 팔 각도 이동")
    ...
    # 노란색의 HSV 범위 설정
    lower_yellow = np.array([20, 100, 100])  # 노란색 하한
    upper_yellow = np.array([30, 255, 255])  # 노란색 상한
    ...
    while True:
        ...
        # 노란색 범위 내에 있는 모든 픽셀을 찾아 마스크 생성
        yellow_mask = cv2.inRange(hsv_frame, lower_yellow, upper_yellow)

        # 마스크를 사용해 원본 이미지에서 노란색 부분만 추출
        ...
        if len(contours) == 0:
            print("노란색 탐지 안됨!!")  # 노란색을 탐지하지 못할 때 출력
        else:
            # 노란색이 탐지되었을 경우 타이머 리셋
            start_time = time.time()
        ...
        # 노란색을 10초 동안 탐지하지 못하면 초기 위치로 돌아감
        if time.time() - start_time > 10:
            print("10초 동안 노란색이 탐지되지 않았습니다. 초기 위치로 돌아갑니다.")
            mc.send_angles([0, 0, 0, 0, 0, 0], 20)
            break

        # 결과 이미지와 원본 이미지에 윤곽선 표시
        cv2.imshow('Webcam', frame)
        cv2.imshow('Yellow Color Detection', yellow_result)
...
# 메인 실행
perform_action_and_detect_yellow()

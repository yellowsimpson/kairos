...
def detect_and_grab_block():
    ...
    for attempt in range(10):  # QR 코드 감지를 최대 5회 시도
...
def perform_pose2_adjustments():
    ...
    time_limit = 30  # 10초 제한
    attempts = 5  # 감지 시도 횟수
    max_attempts = 10  # 감지 최대 시도 횟수
...
def detect_and_adjust_position():
    ...
    for result in results:
        for box in result.boxes:
            if box.conf.item() >= CONFIDENCE_THRESHOLD:
                ...
                # 바운딩 박스 및 중심점 표시
                cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)  # 바운딩 박스
                cv2.circle(frame, (x_center, y_center), 5, (0, 0, 255), -1)  # 중심점
                cv2.putText(frame, f"Center: ({x_center}, {y_center})", 
    ...
    cv2.waitKey(1)  # 프레임 업데이트를 위해 1ms 대기
